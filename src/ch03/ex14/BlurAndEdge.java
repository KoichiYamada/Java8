...
public class BlurAndEdge extends Application {
	...
	public void start(final Stage primaryStage) throws Exception {
		final Image origImage = new Image(
				new File("src/ch03/ex13/queen-mary.png").toURI().toString());
		// ブラー
		final ColorTransformer blur = (x, y, l) -> {
			...
			for (int i = 0; i < 3; i++) {
				for (int j = 0; j < 3; j++) {
					final int xx = (x + i) - 1;
					final int yy = (y + j) - 1;
					if ((xx < 0) || (xx >= origImage.getWidth()) || (yy < 0)
							|| (yy >= origImage.getHeight())) {
						continue;
					}
		...
		final ColorTransformer frame = (x, y,
		        l) -> (x < 10) || (x > (origImage.getWidth() - 10)) || (y < 10)
		                || (y > (origImage.getHeight() - 10))
		                        ? Color.GRAY
		                        : l.getColor(x, y);
