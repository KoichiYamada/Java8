...
/*
 * Ｑ：どれだけ安全か Ａ：既存の実装と同じシグニチャだとしても、既存の実装が優先されるので、既存の実装を破壊しない。 Ｑ：streamメソッドが古いコードのコンパイルを失敗させるシナリオ
 * Ａ：戻り値がStreamでないstreamを実装済みであればコンパイルエラーとなる Ｑ：JARの古いコードは動作するか Ａ：動作する（中間コードへは影響がない。実験でも動作することが確認できた）
